...
parser.add_argument('--stop', type=int, default=20,
                    help='The number of epochs to stop.')
...
parser.add_argument('--arch_pool', type=str, default=None,
                    help='Dags to sample from. If None for default, then randomly sampled arch is used.')

parser.add_argument('--arch_pool_perf', type=str, default=None,
                    help='perf of dags to sample from. If None for default, then randomly sampled arch is used.')
...
def get_params():
  ...
  if FLAGS.arch_pool_perf is not None:
    with open(FLAGS.arch_pool_perf) as f:
      perfs = f.read().splitlines()
      probs = list(map(lambda x:1-float(x), perfs))
      params['arch_pool_prob'] = probs
